...
import {
  ...
  getConexoesDisparo, getHorariosDisparo, type HorarioDisparo,
...
export function EnviarGrupoWizard({
  ...
  // continua sendo sempre na mão (regra do dono, depois dos incidentes de
  ...
  useEffect(() => {
    carregarStatus();
    carregarConexoes();
    // Horários fixos do gestor. A 1ª chamada semeia os padrões, então a lista
    // nunca volta vazia por falta de configuração.
    getHorariosDisparo()
      .then(setHorarios)
      .catch(() => { /* sem lista, o passo 5 cai no campo de hora */ });
  }, []);
  ...
  // ── Horários pré-definidos (mesma mecânica do disparo normal) ───────────────

  // Sem lista configurada não há o que oferecer — cai no campo de hora.
  const preset = usarPreset && horarios.length > 0;
  const horariosOrdenados = useMemo(() => [...horariosSel].sort(), [horariosSel]);

  /** Liga/desliga os chips. Ao ligar já marca o primeiro, senão fica sem hora. */
  ...
  function alternarHorario(h: string) {
    setHorariosSel((atual) => {
      const novo = new Set(atual);
      if (novo.has(h)) novo.delete(h); else novo.add(h);
      return novo;
    });
  }
   ...
   * vira o `agendado_para`. Marcar 08:00 e 18:00 às 10h tem que começar às
   * 18:00 de hoje, não disparar o 08:00 atrasado na hora.
   */
  ...
  // Grupos cujo nome carrega TODOS os tokens distintivos do cliente. Sem
  // cliente escolhido (envio avulso) não há por quem filtrar.
  const combinam = useMemo(
    // Na carteira a razão social vem em `farmacia`; o helper espera `nome`.
    () => (clienteSel
      ? grupos.filter((g) => combinaComFarmacia(g.nome, {
          nome: clienteSel.farmacia, nome_fachada: clienteSel.nome_fachada,
        }))
      : []),
    [grupos, clienteSel],
  );

  // Nenhum grupo casou com o nome do cliente: abre na lista inteira, senão o
  // gestor encara uma tela vazia sem saber que existe o "Ver todos".
  useEffect(() => {
    if (grupos.length > 0 && combinam.length === 0) setSoDoCliente(false);
  }, [grupos, combinam]);

  const filtrandoPorCliente = soDoCliente && !!clienteSel && combinam.length > 0;

  const gruposFiltrados = useMemo(() => {
    const base = filtrandoPorCliente ? combinam : grupos;
    const filtro = busca.trim().toLowerCase();
    return filtro ? base.filter((g) => g.nome.toLowerCase().includes(filtro)) : base;
  }, [filtrandoPorCliente, combinam, grupos, busca]);
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {etapa === 4 && (
            <div className="flex flex-col gap-3">
              {clienteSel && grupos.length > 0 && (
                <div className="flex items-center gap-3 flex-wrap bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-zinc-900 truncate">
                      Grupos de {clienteSel.farmacia_visivel ?? clienteSel.farmacia}
                    </p>
                    <p className="text-[11px] text-zinc-500 mt-0.5">
                      {filtrandoPorCliente
                        ? `Mostrando ${combinam.length} grupo(s) com o nome do cliente.`
                        : combinam.length === 0
                          ? `Nenhum grupo tem o nome do cliente — mostrando todos os ${grupos.length}.`
                          : `Mostrando todos os ${grupos.length} grupo(s) da conexão.`}
                    </p>
                  </div>
                  {combinam.length > 0 && (
                    <button
                      onClick={() => setSoDoCliente((v) => !v)}
                      className="text-sm font-medium text-brand hover:underline shrink-0"
                    >
                      {filtrandoPorCliente ? "Ver todos os grupos" : "Só os deste cliente"}
                    </button>
                  )}
                </div>
              )}
              ...
              {carregandoGrupos ? (
                ...
                <div className="grid gap-2 max-h-80 overflow-y-auto">
                  {gruposFiltrados.map((g) => {
                    ...
                    const pareceDoCliente = combinam.some((c) => c.jid === g.jid);
                    return (
                      <button
                        key={g.jid}
                      ...
                      >
                        ...
                        {pareceDoCliente && !filtrandoPorCliente && (
